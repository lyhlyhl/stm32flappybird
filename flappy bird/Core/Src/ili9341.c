...
{
    // 获取屏幕中心坐标
    uint16_t cx = lcdProperties.width / 2;
    uint16_t cy = lcdProperties.height / 2;
    
    // 调整缩放比例使爱心适合屏幕大小
    uint16_t scale = (cx < cy ? cx : cy) / 2;

    // 遍历爱心可能出现的区域（优化遍历范围）
    for (int16_t y = -2*scale; y <= 2*scale; y++) {      // 数学坐标系y范围
        for (int16_t x = -2*scale; x <= 2*scale; x++) { // 数学坐标系x范围
            // 转换为数学坐标系（屏幕y轴方向需要翻转）
            float xf = (float)x / scale;
            float yf = (float)(-y) / scale;  // 反转y轴方向
            
            // 心形方程：(x² + y² - 1)³ - x²y³ ≤ 0
            float temp = xf*xf + yf*yf - 1;
            temp = temp * temp * temp;
            if (temp - (xf*xf * yf*yf*yf) <= 0) {
                // 转换为屏幕坐标系并绘制
                uint16_t screenX = cx + x;
                uint16_t screenY = cy + y;
                lcdDrawPixel(screenX, screenY, color);
            }
        }
    }
}
...
void MylcdWriteData(unsigned short data)
{
	LCD_DataWrite(data);
}
